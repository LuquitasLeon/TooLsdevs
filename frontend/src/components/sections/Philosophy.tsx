      ...
      <Container className="relative flex flex-col items-center gap-8 text-center">
        ...
        <Reveal delay={0.1} className="max-w-2xl flex flex-col gap-4">
          {philosophy.paragraphs.map((p) => (
            <p key={p} className="text-base sm:text-lg leading-relaxed text-slate-200/90">
              {p}
            </p>
          ))}
        </Reveal>
        ...
        <Reveal delay={0.2}>
          <p className="font-display text-3xl sm:text-5xl font-bold text-white max-w-3xl text-balance leading-tight">
